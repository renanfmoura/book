...
\end{lstlisting}
        ...
        Ora, sabemos que $x$ representa uma "espaço reservado" para toda variável do domínio. O que representa $y$?
        Representa um elemento específico do domínio. Digamos que o domínio é $\mathbb{N}$, então podemos considerar o elemento zero como $y$.
        Sendo assim, se trocarmos $y$ por $z$ (que pode representar outro elemento), então não necessariamente vale a expressão
        $\forall x z \le x$. Ora, $z$ pode representar o elemento um, então a expressão seria falsa.
        ...
        é que, se $A(x)$ vale para todo $x$, então vale para algum $t$ particular do domínio.
        ...
        Ora, e como representar isso em Lean? Esta é a regra de introdução para o quantificador universal:
...
\end{lstlisting}
        ...
        Estamos mostrando $\forall x A(x)$ assumindo um $y$ qualquer e mostrando que vale $A(y)$.
Neste caso, necessitaríamos de uma prova de que $A(y)$, por isso o uso do \lstinline{sorry}.
...
\end{lstlisting}
        ...
        Dado que sabemos que $\forall x A(x)$, podemos "aplicar" $t$ e obter $A(t)$.
